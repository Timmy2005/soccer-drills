<template>
	<div id="register">
		...
		<form id="password-form" @submit.prevent="register" spellcheck="false">
			...
			<div class="button-container">
				...
				<button id="register-button" class="mdc-button mdc-button--unelevated" type="submit" v-button>
					<div class="mdc-button__ripple"></div>
					<span v-if="!processing" class="mdc-button__label">Register</span>
...
<script>
	...
	export default {
		...
		methods: {
			register() {
				...
				this.$http.post('/register-user/', {
					...
					'password': this.password,
						...
						if (response.data.registration === 'valid') {
							window.location = response.data.url
						} else {
							that.passwordErrorText = 'Username already exists'
			...
			backToLogin() {
				this.$store.dispatch('setRoute', 0)
			}
		},
		directives: {
			textField: {
